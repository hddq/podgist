...
WHERE user_id = ? 
  ...
  AND (sqlc.narg('podcast_url_filter') IS NULL OR podcast_url = sqlc.narg('podcast_url_filter'))
  AND (sqlc.narg('device_id_filter') IS NULL OR device_id = sqlc.narg('device_id_filter'))
...
FROM subscriptions
WHERE user_id = ? AND device_id IN (sqlc.slice('device_ids'))
ORDER BY podcast_url;
...
DELETE FROM sync_group_members WHERE sync_group_id IN (sqlc.slice('group_ids'));

-- name: DeleteSyncGroupsByIDs :exec
DELETE FROM sync_groups WHERE user_id = ? AND id IN (sqlc.slice('group_ids'));
